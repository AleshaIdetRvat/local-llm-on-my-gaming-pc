...
- **⚠️ KV-квант только `q4_0` или `q8_0`, и K с V — ОДНОГО типа.** У CUDA flash
  attention при `head_dim 256` нет быстрого кернела ни для `q5_1`, ни для смешанных
  K/V: prefill 1437 → **108 t/s** при здоровой VRAM. Выглядит как спилловер, но им
  не является — подробности в
  [docs/choosing-models.md](../../docs/choosing-models.md#️-тип-kv-кванта-выбирай-из-тех-что-умеет-flash-attention).
  Проверенные варианты (2026-08-27, ctx 32K, VRAM 13.2 из 16.3 ГБ):
  ...
  То есть популярный совет «K точнее, V грубее» здесь **неприменим**: он стоил бы
  порядка скорости. Если бы работал — дал бы ~100K контекста (KV 2600 МиБ).
...
- **Vision включён 2026-08-29, проектор считает на CPU.** `mmproj-F16.gguf` (927 607 488 Б)
  из того же репо, флаги `--mmproj … --no-mmproj-offload`. Держать проектор в VRAM негде:
  пик и без него 15467 из 16303. С ним пик 15572 — проектор на GPU действительно не едет.
  Текстовая скорость не изменилась: pp 1539 @3.4K, tg 50.0 (было 1744 @2K / 49.4).
  **Цена — время на картинку, растёт быстрее числа токенов:**
  ...
  То есть картинку вчетверо больше по площади проектор считает вдесятеро дольше —
  на CPU узкое место сам ViT, а не число токенов на выходе. Практический вывод:
  ужимать до ~448–768 по длинной стороне, полноразмерный скриншот — это минута ожидания.
  Качество не страдает: тестовую картинку (3 синих круга + красный квадрат) описывает верно.
- Не сделано: MTP-драфтер (1.37 ГБ — VRAM нет), слепое сравнение качества
  с прод-`qwen3.6-35b-a3b`.
