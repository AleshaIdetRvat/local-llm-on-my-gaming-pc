...
>
> Обновлено **2026-08-27** (зеркала синхронизированы с ПК по scp в этот же день).
...
- У всех моделей заполнены `name` / `description` / `capabilities` — `/v1/models` отдаёт
  клиентам контекст, модальности и tool calling, так что список моделей у клиента
  не хранится (см. [llama-swap.md](../../docs/llama-swap.md#метаданные-моделей-пусть-клиент-сам-узнаёт-что-есть-на-стенде)).
...
Колонка «Контекст» — значение `-c`. Клиенту в `capabilities.context` уходит потолок
**одного** запроса: у пары `gemma-4-26b-a4b-qat*` это 16K, потому что `-c 32768` делится
между двумя слотами `-np 2`.

Клиент pi (`@earendil-works/pi-coding-agent`) на макбуке берёт список моделей прямо из
`/v1/models` — расширение `~/.pi/agent/extensions/rig-models.ts`, шаблон в репо:
[`templates/pi/models-from-llama-swap.ts`](../../templates/pi/models-from-llama-swap.ts).
Новая модель на стенде появляется у него сама.
...
форка, макрос не использует) плюс `name` / `description` / `capabilities` для клиентов. На ПК лежат бэкапы прежних версий: `config.yaml.bak*`.
